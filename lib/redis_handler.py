...
def config_cache(config, path):
    ...
    print(Colors.FG.LightGreen + Colors.Bold + action + " Started" + Colors.Reset)
    shell.run_bash_command_popen(config, path, action, "php -ddisplay_errors=on " + path + "/bin/magento setup:config:set --page-cache=redis --page-cache-redis-server=redis --page-cache-redis-db=0 --page-cache-redis-port=6379")
    menu.main_menu(path)
