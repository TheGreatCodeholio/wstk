...
def backup_to_s3_bucket(config, path):
    x = datetime.datetime.now()
    current_date = x.strftime("%d%B%y")
    action = "S3 Bucket Backup"
    ...
    shell.run_bash_command_popen(config, path, action, "aws s3 cp /srv/backup_" + current_date + ".tar.gz s3://" + s3_bucket)
    action = "Backup Cleanup"
    print(Colors.FG.LightGreen + "Starting " + action + Colors.Reset)
    action = "Remove Backup Archive"
    shell.run_bash_command_popen(config, path, action, "rm -f /srv/backup_" + current_date + ".tar.gz")
    action = "Remove Backup Database"
    shell.run_bash_command_popen(config, path, action, "rm -rf " + path + "/database")
    menu.magento_backup_menu(config, path)

def backup_local_auto(config, path, media):
    x = datetime.datetime.now()
    current_date = x.strftime("%d%B%y")
...
def backup_local_custom(config, path, media):
    x = datetime.datetime.now()
    current_date = x.strftime("%d%B%y")
    action = "Magento Local Backup"
    print(Colors.FG.LightGreen + "Starting " + action + Colors.Reset)
