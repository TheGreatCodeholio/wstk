...
def config_sessions(config, path):
    if config["session"]["save"] == "redis":
        print(Colors.OKGREEN + "Sessions already using Redis." + Colors.ENDC)
        Menu.main_menu(config, path)
    else:
        if config["session"]["save"]:
...
def config_cache(config, path):
    if config["cache"]["frontend"]["default"]["backend"] == "Cm_Cache_Backend_Redis":
        print(Colors.OKGREEN + "Cache already using Redis." + Colors.ENDC)
        Menu.main_menu(config, path)
    else:
        del config["cache"]
        config["cache"] = {}
